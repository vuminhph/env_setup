...
if [ "$OS" == "Linux" ]; then
	...
	echo -e "${RED}$OS platform is not supported currently${RESET}"
	exit 1
fi
...
function download_neovim() {
	echo "Downloading NeoviLV_INSTALL_PREFIXm's binary from $RELEASE_VER release.."
